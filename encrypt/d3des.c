...
static void scrunch(uint8_t *, uint32_t *);
static void unscrun(uint32_t *, uint8_t *);
...
static void cookey(uint32_t *raw1)
{
    uint32_t *cook, *raw0;
    uint32_t dough[32];
    int i;
...
void des(uint8_t *inblock, uint8_t *outblock)
{
    uint32_t work[2];
...
static void scrunch(uint8_t *outof, uint32_t *into)
{
    *into    = (*outof++ & 0xffL) << 24;
    *into   |= (*outof++ & 0xffL) << 16;
    *into   |= (*outof++ & 0xffL) << 8;
...
static void unscrun(uint32_t *outof, uint8_t *into)
{
    *into++ = (*outof >> 24) & 0xffL;
    *into++ = (*outof >> 16) & 0xffL;
    *into++ = (*outof >>  8) & 0xffL;
...
void Ddes(uint8_t *from, uint8_t  *into     /* uint8_t[8] */)
{
    uint32_t work[2];
    scrunch(from, work);
    desfunc(work, KnL);
...
void D2des(
    uint8_t *from,           /* uint8_t[16] */
    uint8_t *into            /* uint8_t[16] */
)
{
    uint32_t *right, *l1, swap;
    uint32_t leftt[2], bufR[2];
    right = bufR;
    l1 = &leftt[1];
    scrunch(from, leftt);
...
void cp2key(uint32_t *into    /* uint32_t[64] */)
{
    uint32_t *from, *endp;
    cpkey(into);
    into = &into[32];
...
void cp3key(uint32_t *into    /* uint32_t[96] */)
{
    uint32_t *from, *endp;
    cpkey(into);
    into = &into[32];
...
void use3key(uint32_t *from    /* uint32_t[96] */)
{
    uint32_t *to, *endp;
    usekey(from);
    from = &from[32];
...
static void D3des(uint8_t *, uint8_t *);

static void D3des(
    uint8_t *from,            /* uint8_t[24] */
    uint8_t *into            /* uint8_t[24] */
)   /* amateur theatrics */
{
    uint32_t swap, leftt[2], middl[2], right[2];
    scrunch(from, leftt);
